...
export function HabitDay({ defaultCompleted = 0, amount = 0, date }: HabitProps) {
  const [completed, setComplete] = useState(defaultCompleted)
  ...
  function handleCompletedChange(completedArg: number) {
    setComplete(completedArg)
  }
  ...
  return(
    <Popover.Root>
      ...
      <Popover.Portal>
        <Popover.Content className='min-w-[320px] max-h-96 overflow-y-auto p-6 rounded-2xl bg-zinc-900 flex flex-col'>
